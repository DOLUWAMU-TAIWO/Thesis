...
void test_print_float_to_uart() {
    print_float_to_uart(123.456); // Test a positive float
    kputc('\n');                  // New line for clarity

    print_float_to_uart(-78.91011); // Test a negative float
    kputc('\n');                   // New line for clarity

    print_float_to_uart(0.001234); // Test a small float
    kputc('\n');                   // New line for clarity

    print_float_to_uart(123456.0); // Test a large float
    kputc('\n');                   // New line for clarity
}
...
int main(void)
{
    ...
    if (arsize_input == NULL) {
        ...
        kprintf("Memory required:  %ldK.\n",(memreq+512L)>>10);
        malloc_arg=(size_t)memreq;
        if (malloc_arg!=memreq || (mempool=malloc(malloc_arg))==NULL)
            {
            kprintf("Not enough memory available for given array size.\n\n");
            return 2;
            }

        kprintf("\n\nLINPACK benchmark, %s precision.\n",PREC);
        kprintf("Machine precision:  %d digits.\n",BASE10DIG);
        kprintf("Array size %d X %d.\n",arsize,arsize);
        kprintf("Average rolled and unrolled performance:\n\n");
        kprintf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS\n");
        kprintf("----------------------------------------------------\n");
        nreps=1;
...
static REAL linpack(long nreps,int arsize)
    ...
    kprintf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f\n",
